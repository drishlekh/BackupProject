...
class MainActivity : AppCompatActivity() {
            ...
            override fun onServiceFound(serviceInfo: NsdServiceInfo) {
                ...
                nsdManager.resolveService(serviceInfo, object : NsdManager.ResolveListener {
                    ...
                    override fun onServiceResolved(serviceInfo: NsdServiceInfo) {
                        ...
                        val port = serviceInfo.port
                        val address = host.hostAddress
                        Log.d("NSD", "Resolved service: ${serviceInfo.serviceName} - IP Address: $address - Host: $host - Port: $port")
                        if (address != null) {
                            discoveredIPAddress.add(address)
                            connectToService(address, port)
                        }
                        Log.d("NSD", "Discovered IP Addresses: $discoveredIPAddress")
                        mainHandler.post {
                            if (discoveredServices.add(serviceInfo)) {
    ...
    private fun handleClient(clientSocket: Socket) {
        Thread {
            try {
                ...
                // Add protocol handling code here
                writer.println("SYNC_MUSIC")
    ...
    private fun connectToService(address: String, port: Int) {
        Thread {
            try {
    ...
    private fun showAdvertiseDialog() {
        AlertDialog.Builder(this)
            .setTitle("Advertise")
            .setMessage("Stop advertising this device?")
            .setPositiveButton("Stop") { _, _ ->
                stopAdvertisingService()
            }
